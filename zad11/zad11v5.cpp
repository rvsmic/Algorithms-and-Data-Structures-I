...
//porownywanie rosnaco wg id, nazwiska i imienia
...
inline uint find_median(Person*** people, uint start, uint end) {
  ...
  if(!((end - start) & 1) && people_comparator((*people)[(start + end) / 2],(*people)[((start + end) / 2) - 1])) {
...
inline void find_element(Person*** people, uint start, uint end, uint* k) {
  ...
  for(uint i=start;i<end;++i) {
    if(people_comparator((*people)[i],median_person)) {
      tmp = (*people)[start + B_end];
      (*people)[start + B_end] = (*people)[i];
      (*people)[i] = tmp;
      ++B_end;
    }
  }
  for(uint i=start + B_end;i<end;++i) {
    if((*people)[i]->id == median_person->id && (*people)[i]->last_name == median_person->last_name && (*people)[i]->first_name == median_person->first_name) {
      tmp = (*people)[start + B_end + C_end];
      (*people)[start + B_end + C_end] = (*people)[i];
      (*people)[i] = tmp;
      ++C_end;
    }
  }

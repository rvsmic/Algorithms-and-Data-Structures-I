...
struct List {
    ...
    void return_to_game(List* first_out, List* last_out, short direction) {
        ...
        // List* first_copy = first;
        // List* second_copy = second;

        // cout<<"-"<<first_copy->data.number<<first_copy->data.side<<"-";
        // cout<<"-"<<second_copy->data.number<<second_copy->data.side<<"-";

        //usuniecie z kolejki znalezionej pary
        if(first != first_out) {
            //przypisanie odniesienia do zmiennej i tryb pracy (0 - jak na poczatku, 1 - jak na koncu)
            first->pop_element(first, 2);
        } else {
            first->pop_element(first_out, 0);
        }

        if(second != last_out) {
            second->pop_element(second,2);
        } else {

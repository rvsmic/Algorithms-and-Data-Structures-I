...
struct List {
    ...
    void add_out(List** new_out, List** first_out, List** last_out, bool *delete_arr, short* delete_count) {
        if(delete_arr[(*new_out)->num] == true) {
    ...
    void join_game(List** first_out, bool* delete_arr, short* delete_count, short direction, short* count) {
        ...
        *first_out = (*first_out)->pop_first();
        ...
        this->join_chain(new List(number),direction,0);
        this->join_chain(new List(number),direction,1);
...
int main() {
    ...
    List* first_pair = new List(0);
    List* current = new List(first_pair,n-1);
    ...
    for(int i=1;i<n;i++) {
        current = new List(current,i);
        current = new List(current,(i-1)%n);
        if(i == p) {
            start = current->prev;
        }
        delete_arr[i] = false;

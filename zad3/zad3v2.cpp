...
struct List {
    ...
    void advanced_pop_element(List* deleted, List* new_current, short direction) {
        //ustawienie nowego wskaznika
        deleted = this;
        next->previous = previous;
        previous->next = next;
        if(direction == 1) {    //nie na odwrot?
            new_current = this->previous;
        } else {
            new_current = this->next;
        }

        cout<<"del("<<deleted->data.number<<deleted->data.side<<")\n";
    }
    ...
    void return_to_game(List* first_out, List* last_out, short direction) {
        ...
        if(first != first_out) {
            //przypisanie odniesienia do zmiennej i tryb pracy (0 - jak na poczatku, 1 - jak na koncu)
...
int main() {
    ...
    for(int i=0;i<m;i++) {
        ...
        if(op == 0) {
            ...
            if(n*2==count) {
                current->advanced_pop_element(first_out,current,k);
                ...
                out->add_out(current);
                current->advanced_pop_element(out, current,k);

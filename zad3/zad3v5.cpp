    ...
    void return_to_game(List* first_out, List* last_out, short direction) {
        ...
        while(first->data.number != second->data.number) {
            while(second != last_out) {
                if(first->data.number == second->data.number && first->data.side != second->data.side) {
                    found = true;
                    break;
                }
                second = second->next;
        ...
        if(first->data.number == second->data.number && first->data.side != second->data.side) {
            ...
            //cout<<"("<<first->data.number<<first->data.side<<" "<<second->data.number<<second->data.side<<")\n";
        }
        ...
        if(found) {
            ...
            //czy jest z lewej, czy jest z prawej, czy sa po sobie
...
int main() {
    ...
    for(int i=0;i<m;i++) {
        ...
        if(op == 0) {
            ...
            //cout<<"~"<<first_out->data.number<<first_out->data.side<<" - "<<out->data.number<<out->data.side<<"~\n";

...
inline void pop(Op** operations, Op** opposite_operations, int *operation_count, bool max_mode) {
  ...
  *operation_count -= 1;
  ...
  if(max_mode) {
    for(int i=old_root->min_index;i<*operation_count;++i) {
      opposite_operations[i] = opposite_operations[i+1];
      opposite_operations[i]->min_index = i;
    }
  } else {
    for(int i=old_root->max_index;i<*operation_count;++i) {
      opposite_operations[i] = opposite_operations[i+1];
      opposite_operations[i]->max_index = i;
    }
  }

  sift_up(opposite_operations,*operation_count,!max_mode);

...
int sift_down(Op* operations, Op* opposite_operations, int operation_count, bool max_mode) {
  ...
  return operations[index - 1].opposite_index;
}
...
void pop(Op* operations, Op* opposite_operations, int *operation_count, bool max_mode) {
  ...
  opposite_operations[old_root.opposite_index] = opposite_operations[operations[0].opposite_index];
...
//   cout<<"O: "<<old_root.fix_priority<<"\n";
  *operation_count -= 1;
  ...
  node_fix_index = sift_down(operations,opposite_operations,*operation_count,max_mode);
